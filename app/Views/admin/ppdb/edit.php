            ...
            <div class="card border-0 shadow-sm mb-4">
                <div class="card-body p-4">
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" name="judul_blok" value="<?= esc(old('judul_blok', $konten['judul_blok'])) ?>" required>
                    </div>
                    <div>
                        <label class="form-label fw-semibold">Konten <span class="text-danger">*</span></label>
                        <div id="quill-editor"></div>
                        <textarea name="konten" id="kontenInput" class="d-none"><?= old('konten', $konten['konten']) ?></textarea>
                    </div>
                </div>
            </div>
        </div>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="tipe" class="form-select" required>
                            <?php foreach (['persyaratan' => 'Persyaratan', 'jadwal' => 'Jadwal', 'alur' => 'Alur Pendaftaran', 'faq' => 'FAQ', 'info' => 'Info'] as $v => $l): ?>
                                <option value="<?= $v ?>" <?= old('tipe', $konten['tipe']) === $v ? 'selected' : '' ?>><?= $l ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div class="mb-3">
                        <label class="form-label">Urutan</label>
                        <input type="number" class="form-control" name="urutan" value="<?= esc(old('urutan', $konten['urutan'])) ?>" min="0">
                    </div>
                    <div class="form-check form-switch">
                        <input class="form-check-input" type="checkbox" id="isActive" name="is_active" value="1"
                            <?= old('is_active', $konten['is_active']) ? 'checked' : '' ?>>

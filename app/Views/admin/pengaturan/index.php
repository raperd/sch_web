...
<form method="post" action="<?= admin_url('pengaturan/update') ?>" enctype="multipart/form-data" id="settingsForm" data-no-progress>
    <?= csrf_field() ?>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <?php elseif ($setting['tipe'] === 'image'): ?>
                                            ...
                                            <input type="file"
                                                ...
                                                data-aspect-ratio="<?= $key === 'foto_kepsek' || $key === 'favicon_path' ? '1' : ($key === 'hero_image_path' ? '1.7778' : '1') ?>"
...
<script>
    ...
    const cropTitles = {
        ...
        logo_path: 'Crop Logo Sekolah (1:1)',
    ...
    document.getElementById('cropConfirmPengaturan')?.addEventListener('click', function() {
        ...
        cropperPengaturan.getCroppedCanvas({
            width: w,
            height: h,
            imageSmoothingQuality: 'high'
        }).toBlob(blob => {
            const file = new File([blob], 'foto-crop.' + ext, {
                type: mimeType
            });
            ...
            if (newPrev) {
                newPrev.querySelector('img').src = URL.createObjectURL(blob);
                newPrev.classList.remove('d-none');
            }
    ...
    document.getElementById('cropModalPengaturan').addEventListener('hidden.bs.modal', function() {
        if (cropperPengaturan) {
            cropperPengaturan.destroy();
            cropperPengaturan = null;
        }
...
<script>
            ...
            reader.onload = ev => {
                ...
                img.onload = () => {
                    ...
                    canvas.toBlob(blob => {
                        ...
                        compressedBlobs.set(capturedInput, {
                            file: compressed,
                            fieldName: capturedInput.name
                        });
    ...
    function saveThemeAjax(primary, secondary, accent) {
        ...
        fetch('<?= admin_url('pengaturan/update') ?>', {
                method: 'POST',
                body: fd
            })
            .then(() => {
                ...
                setTimeout(() => {
                    if (btn) btn.innerHTML = '<i class="bi bi-save me-1"></i>Terapkan ke Website';
                }, 2000);
            })
            .catch(() => {
                if (btn) {
